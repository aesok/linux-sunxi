...
static int mt9d112_power_on(struct sensor_info *info)
{
...
static int mt9d112_power_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int mt9d112_stby_on(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int mt9d112_stby_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	switch(on)
	{
		case CSI_SUBDEV_STBY_ON:
			mt9d112_stby_on(info);

			break;
		case CSI_SUBDEV_STBY_OFF:
			mt9d112_stby_off(info);
 ...
 */
static int mt9d112_g_mbus_config(struct v4l2_subdev *sd,
...
static const struct v4l2_subdev_video_ops sensor_video_ops = {
	...
	.g_mbus_config = mt9d112_g_mbus_config,
};
