...
static int gc0308_power_on(struct sensor_info *info)
{
...
static int gc0308_power_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int gc0308_stby_on(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int gc0308_stby_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	switch(on)
	{
		...
		case CSI_SUBDEV_STBY_OFF:
			gc0308_stby_off(info);

			break;
		case CSI_SUBDEV_PWR_ON:
			gc0308_power_on(info);
...
static int gc0308_registered(struct v4l2_subdev *sd)
{
	struct sensor_info *info = to_state(sd);
	int ret;

	v4l2_dbg(1, debug, sd, "gc0308_registered.\n");
	...
	if (ret) {
		v4l2_err(sd, "chip found is not an gc0308 chip.\n");
	} else {
		v4l2_info(sd, "the gc0308 chip is found.\n");
	}
