...
//	struct v4l2_pix_format *pix = &fmt->fmt.pix;//linux-3.0
	 ...
	 */
	fmt->width = wsize->width;//linux-3.0dev
 ...
 */
static int gc0309_g_mbus_config(struct v4l2_subdev *sd,
...
static int gc0309_registered(struct v4l2_subdev *sd)
{
	struct sensor_info *info = to_state(sd);
	int ret;

	v4l2_dbg(1, debug, sd, "gc0309_registered.\n");
	...
	if (ret) {
		v4l2_err(sd, "chip found is not an gc0309 chip.\n");
	} else {
		v4l2_info(sd, "the gc0309 chip is found.\n");
	}
...
static const struct v4l2_subdev_video_ops sensor_video_ops = {
	...
	.g_mbus_config = gc0309_g_mbus_config,
};
...
static const struct v4l2_subdev_internal_ops gc0309_internal_ops = {
	.registered = gc0309_registered,
};
			...
			const struct i2c_device_id *id)
{
	...
	sd->internal_ops = &gc0309_internal_ops;
