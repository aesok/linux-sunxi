...
static int mt9m112_power_on(struct sensor_info *info)
{
...
static int mt9m112_power_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int mt9m112_stby_on(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
	int ret;
	...
	ret = sensor_write_array(&info->sd, sensor_oe_disable_regs , ARRAY_SIZE(sensor_oe_disable_regs));
	if(ret < 0)
		v4l2_err(&info->sd,"disalbe oe falied!\n");
	//standby on io
	sensor_gpio_write(dev->csi_pin_hd, info->stby, CSI_STBY_ON);
	msleep(100);
//	sensor_gpio_write(dev->csi_pin_hd, info->stby, CSI_STBY_OFF);
...
static int mt9m112_stby_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	switch(on)
	{
		case CSI_SUBDEV_STBY_ON:
			mt9m112_stby_on(info);

			break;
		case CSI_SUBDEV_STBY_OFF:
			mt9m112_stby_off(info);

			break;
		case CSI_SUBDEV_PWR_ON:
			mt9m112_power_on(info);

			break;
		case CSI_SUBDEV_PWR_OFF:
			mt9m112_power_off(info);
 ...
 */
static int mt9m112_g_mbus_config(struct v4l2_subdev *sd,
...
static int mt9m112_registered(struct v4l2_subdev *sd)
{
	struct sensor_info *info = to_state(sd);
	int ret;

	v4l2_dbg(1, debug, sd, "mt9m112_registered.\n");
	...
	if (ret) {
		v4l2_err(sd, "chip found is not an mt9m112 chip.\n");
	} else {
		v4l2_info(sd, "the mt9m112 chip is found.\n");
	}
...
static const struct v4l2_subdev_video_ops sensor_video_ops = {
	...
	.g_mbus_config = mt9m112_g_mbus_config,
};
...
static const struct v4l2_subdev_internal_ops mt9m112_internal_ops = {
	.registered = mt9m112_registered,
};
			...
			const struct i2c_device_id *id)
{
	...
	sd->internal_ops = &mt9m112_internal_ops;
