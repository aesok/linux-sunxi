...
static int ov5640_power_on(struct sensor_info *info)
{
...
static int ov5640_power_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int ov5640_stby_on(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
	int ret;
...
static int ov5640_stby_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	switch(on)
	{
		...
		case CSI_SUBDEV_STBY_OFF:
			ov5640_stby_off(info);

			break;
		case CSI_SUBDEV_PWR_ON:
			ov5640_power_on(info);
...
static int ov5640_registered(struct v4l2_subdev *sd)
{
	...
	v4l2_dbg(1, debug, sd, "ov5640_registered.\n");
	...
	if (ret) {
		v4l2_err(sd, "chip found is not an ov5640 chip.\n");
	} else {
		v4l2_info(sd, "the ov5640 chip is found.\n");
	}
