...
static int hi704_power_on(struct sensor_info *info)
{
...
static int hi704_power_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int hi704_stby_on(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int hi704_stby_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
 ...
 */
static int hi704_g_mbus_config(struct v4l2_subdev *sd,
...
static int hi704_registered(struct v4l2_subdev *sd)
{
	struct sensor_info *info = to_state(sd);
	int ret;

	v4l2_dbg(1, debug, sd, "hi704_registered.\n");
	...
	if (ret) {
		v4l2_err(sd, "chip found is not an hi704 chip.\n");
	} else {
		v4l2_info(sd, "the hi704 chip is found.\n");
	}
...
static const struct v4l2_subdev_video_ops sensor_video_ops = {
	...
	.g_mbus_config = hi704_g_mbus_config,
};
...
static const struct v4l2_subdev_internal_ops hi704_internal_ops = {
	.registered = hi704_registered,
};
			...
			const struct i2c_device_id *id)
{
	...
	sd->internal_ops = &hi704_internal_ops;
