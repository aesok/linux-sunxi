...
static int gc0307_power_on(struct sensor_info *info)
{
...
static int gc0307_power_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int gc0307_stby_on(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
	int ret;
...
static int gc0307_stby_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
 ...
 */
static int gc0307_g_mbus_config(struct v4l2_subdev *sd,
...
static const struct v4l2_subdev_video_ops sensor_video_ops = {
	...
	.g_mbus_config = gc0307_g_mbus_config,
};
