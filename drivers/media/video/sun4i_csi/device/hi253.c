...
static int hi253_power_on(struct sensor_info *info)
{
...
static int hi253_power_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int hi253_stby_on(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int hi253_stby_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
 ...
 */
static int hi253_g_mbus_config(struct v4l2_subdev *sd,
...
static int hi253_registered(struct v4l2_subdev *sd)
{
	struct sensor_info *info = to_state(sd);
	int ret;

	v4l2_dbg(1, debug, sd, "hi253_registered.\n");
	...
	if (ret) {
		v4l2_err(sd, "chip found is not an hi253 chip.\n");
	} else {
		v4l2_info(sd, "the hi253 chip is found.\n");
	}
...
static const struct v4l2_subdev_video_ops sensor_video_ops = {
	...
	.g_mbus_config = hi253_g_mbus_config,
};
...
static const struct v4l2_subdev_internal_ops hi253_internal_ops = {
	.registered = hi253_registered,
};
			...
			const struct i2c_device_id *id)
{
	...
	sd->internal_ops = &hi253_internal_ops;
