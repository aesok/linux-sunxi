...
static int mt9m113_power_on(struct sensor_info *info)
{
...
static int mt9m113_power_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int mt9m113_stby_on(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int mt9m113_stby_off(struct sensor_info *info)
{
	struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(info->sd.v4l2_dev->dev);
...
static int sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	switch(on)
	{
		case CSI_SUBDEV_STBY_ON:
			mt9m113_stby_on(info);

			break;
		case CSI_SUBDEV_STBY_OFF:
			mt9m113_stby_off(info);

			break;
		case CSI_SUBDEV_PWR_ON:
			mt9m113_power_on(info);

			break;
		case CSI_SUBDEV_PWR_OFF:
			mt9m113_power_off(info);
 ...
 */
static int mt9m113_g_mbus_config(struct v4l2_subdev *sd,
...
static int mt9m113_registered(struct v4l2_subdev *sd)
{
	struct sensor_info *info = to_state(sd);
	int ret;

	v4l2_dbg(1, debug, sd, "mt9m113_registered.\n");
	...
	if (ret) {
		v4l2_err(sd, "chip found is not an mt9m113 chip.\n");
	} else {
		v4l2_info(sd, "the mt9m113 chip is found.\n");
	}
...
static const struct v4l2_subdev_video_ops sensor_video_ops = {
	...
	.g_mbus_config = mt9m113_g_mbus_config,
};
			...
			const struct i2c_device_id *id)
{
	...
	sd->internal_ops = &mt9m113_internal_ops;
