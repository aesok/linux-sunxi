...
static int fetch_sensor_config(struct i2c_board_info *binfo)
{
	...
	if((i2c_addr == 0xff) && (strlen(ccm) == 0))	/* when insmod without parm */
	{
		...
		ret = script_parser_fetch("csi0_para", "csi_twi_addr", &tmp_addr, sizeof(int));
		...
		ret = script_parser_fetch("csi0_para", "csi_mname", (int *)&binfo->type, sizeof(binfo->type));
		if (ret) {
	...
	ret = script_parser_fetch("csi0_para","csi_reset", (int *)&sensor_pdata->reset , sizeof(user_gpio_set_t)/sizeof(int));
	if (ret) {
		csi_err("fetch csi_reset from sys_config failed\n");
		return ret;
	}

	ret = script_parser_fetch("csi0_para","csi_stby", (int *)&sensor_pdata->stby , sizeof(user_gpio_set_t)/sizeof(int));
	if (ret) {
		csi_err("fetch csi_stby from sys_config failed\n");
		return ret;
	}

	ret = script_parser_fetch("csi0_para","csi_power_en", (int *)&sensor_pdata->power , sizeof(user_gpio_set_t)/sizeof(int));
	if (ret) {
...
/*
	ret = script_parser_fetch("csi0_para","csi_af_en", (int *)&sensor_pdata->af_power , sizeof(user_gpio_set_t)/sizeof(int));
	if (ret) {
...
*/
	...
	ret = script_parser_fetch("csi0_para","csi_flash", (int *)&sensor_pdata->flash , sizeof(user_gpio_set_t)/sizeof(int));
	if (ret) {
		csi_err("fetch csi_flash from sys_config failed\n");
		return ret;
	}

	ret = script_parser_fetch("csi0_para","csi_flash_pol", (int *)&sensor_pdata->flash_pol , sizeof(int));
	if (ret) {
	...
	ret = script_parser_fetch("csi0_para","csi_if", (int *)&sensor_pdata->interface , sizeof(int));
	if (ret) {
...
static int fetch_sensor_b_config(struct i2c_board_info *binfo)
{
	...
	if((i2c_addr_b == 0xff) && (strlen(ccm_b) == 0))	/* when insmod without parm */
	{
		...
		ret = script_parser_fetch("csi0_para", "csi_twi_addr_b", &tmp_addr, sizeof(int));
		binfo->addr = tmp_addr >> 1;
		if (ret) {
		...
		ret = script_parser_fetch("csi0_para", "csi_mname_b", (int *)&binfo->type, sizeof(binfo->type));
		if (ret) {
	...
	/* fetch camera io configuration */
	ret = script_parser_fetch("csi0_para","csi_reset_b", (int *)&sensor_pdata->reset , sizeof(user_gpio_set_t)/sizeof(int));
	if (ret) {
		csi_err("fetch csi_reset_b from sys_config failed\n");
		return ret;
	}

	ret = script_parser_fetch("csi0_para","csi_stby_b", (int *)&sensor_pdata->stby , sizeof(user_gpio_set_t)/sizeof(int));
	if (ret) {
		csi_err("fetch csi_stby_b from sys_config failed\n");
		return ret;
	}

	ret = script_parser_fetch("csi0_para","csi_power_en_b", (int *)&sensor_pdata->power , sizeof(user_gpio_set_t)/sizeof(int));
	if (ret) {
	...
	/* fetch auto focus io configuration */
/*
	ret = script_parser_fetch("csi0_para","csi_af_en_b", (int *)&sensor_pdata->af_power , sizeof(user_gpio_set_t)/sizeof(int));
	if (ret) {
...
*/
	/* fetch flash io configuration */
	ret = script_parser_fetch("csi0_para","csi_flash_b", (int *)&sensor_pdata->flash , sizeof(user_gpio_set_t)/sizeof(int));
	if (ret) {
		csi_err("fetch csi_flash_b from sys_config failed\n");
		return ret;
	}

	ret = script_parser_fetch("csi0_para","csi_flash_pol_b", (int *)&sensor_pdata->flash_pol , sizeof(int));
	if (ret) {
...
static int fetch_csi_config(struct csi_platform_data *pdata)
{
	...
	ret = script_parser_fetch("csi0_para", "csi_dev_qty", &pdata->dev_qty, sizeof(int));
	if (ret) {
	...
	/* fetch standby mode */
	ret = script_parser_fetch("csi0_para", "csi_stby_mode", &pdata->stby_mode, sizeof(int));
	if (ret) {
	...
	if (pdata->dev_qty > 0) {
		ret = script_parser_fetch("csi0_para", "csi_twi_id", &pdata->i2c_adapter_id[0], sizeof(int));
		if (ret) {
	...
	if (pdata->dev_qty > 1) {
		ret = script_parser_fetch("csi0_para", "csi_twi_id_b", &pdata->i2c_adapter_id[1], sizeof(int));
		if (ret) {
